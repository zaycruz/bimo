...
import { HostToolBridge, parseFrame, VERSION, type Frame } from "./protocol.js";
import { createPiAgent, type AgentFactory } from "./pi-agent.js";

export async function run(factory: AgentFactory = createPiAgent): Promise<void> {
  const configPath = process.env.MONOLITH_WORKER_CONFIG;
  if (!configPath) throw new Error("MONOLITH_WORKER_CONFIG is required");
  const cfg = JSON.parse(fs.readFileSync(configPath, "utf8"));
  let workerSeq = 1;
  let controlSeq = 0;
  let activeRequest: string | undefined;
  let abort: AbortController | undefined;
  let agent: Awaited<ReturnType<AgentFactory>> | undefined;

  const emit = (type: string, payload: Record<string, unknown>, extra: Partial<Frame> = {}) => {
  ...
  const bridge = new HostToolBridge((name, args, id) => {
    if (!activeRequest) throw new Error("no_active_request");
  ...
  for await (const line of readline.createInterface({ input: process.stdin, crlfDelay: Infinity })) {
    ...
    if (frame.factory_gen !== cfg.factory_gen || frame.session !== cfg.session ||
        frame.pod !== cfg.pod.name || frame.seq !== ++controlSeq) throw new Error("stale_frame");

    if (frame.type === "hello_ack") {
      if (agent) throw new Error("duplicate_hello_ack");
      agent = await factory(cfg.pod.config);
    } else if (frame.type === "work_request") {
      if (!agent) throw new Error("work_before_handshake");
      if (activeRequest) throw new Error("concurrent_work");
      ...
      if (frame.request_id !== activeRequest || !frame.tool_call_id) throw new Error("mismatched_tool_result");
